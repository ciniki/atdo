...
// -------
//
function ciniki_atdo_notesSearchQuick($ciniki) {
    //  
    ...
    $rc = ciniki_atdo_checkAccess($ciniki, $args['business_id'], 'ciniki.atdo.notesSearchQuick', 0); 
    if( $rc['stat'] != 'ok' ) { 
        return $rc;
    }   
...
//		. "IF((ciniki_atdos.flags&0x02)=2, 'yes', 'no') AS private, "
	...
	$rc = ciniki_core_dbHashQueryTree($ciniki, $strsql, 'atdo', array(
		array('container'=>'notes', 'fname'=>'id', 'name'=>'note',
			'fields'=>array('id', 'subject', 'assigned', 'assigned_users'), 'lists'=>array('assigned_users')),
		));
	if( $rc['stat'] != 'ok' ) {
		return $rc;
	}
