...
function ciniki_atdo_notesSearchFull($ciniki) {
    ...
    if( (!isset($args['completed']) || $args['completed'] != 'yes') 
        ...
        return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.atdo.15', 'msg'=>'No search specified'));
    }
    ...
    $rc = ciniki_atdo_checkAccess($ciniki, $args['tnid'], 'ciniki.atdo.notesSearchFull', 0); 
    if( $rc['stat'] != 'ok' ) { 
        return $rc;
    }   

    //
    // Get the number of notes in each status for the tenant, 
